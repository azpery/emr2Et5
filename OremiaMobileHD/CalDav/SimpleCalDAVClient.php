...
class SimpleCalDAVClient {
	...
	function icsToArray($array,$icsFile) {
        ...
        foreach($value as $subKey => $subValue) {
            ...
            if ($subValue != "") {
                ...
                if ($key != 0 && $subKey == 0) {
                    ...
                    $subValueArr = explode(":", $subValue, 2);
										$subValue = explode(";", $subValueArr[0], 2);
										if($subValueArr[0] == "UID"){
											$id = explode("\r", $subValueArr[1], 2);
										}
										if($subValue[0] == "RESOURCES"){
                    	$array[$id[0]] = $subValue[1];
									}
									if($cpt == sizeof($value)-1 && !isset($array[$id[0]])){
										$array[$id[0]] = "";
									}
                }
            }
	...
	function getClientEvents($events) {
		// var_dump($events);
		$parsedEvents = array();
    foreach($events as $event) {
        foreach($event as $key => $subValue) {
            if ($key == "data") {
							// var_dump($key);
              $parsedEvents = $this->icsToArray($parsedEvents,$subValue);
            }
        }
    }
    return json_encode($parsedEvents);
	}
	function createEvent($UID,$dtstart,$dtend,$summary,$note,$ressources){
		$datenow =  date("Ymd")."T".date("his")."Z";
		// echo $datenow;die;
...
CREATED:$datenow
DTSTAMP:$datenow
LAST-MODIFIED:20160314T114357Z
$ressources
SEQUENCE:0
$summary
TRANSP:OPAQUE
END:VEVENT
END:VCALENDAR";
		return $this->create($changedFirstEvent);
	}
}
